...
inline Vec3 Vec3FromString(const string& sVector)
{
	const size_t yPos = sVector.find(",");
	const size_t zPos = sVector.find(",", yPos + 1);
...
inline string Vec3ToString(Vec3 vector, bool isShort = true)
{
	string sVector;
	if (!isShort)
		sVector.Format("%f,%f,%f", vector.x, vector.y, vector.z);
	else
		sVector.Format("%.2f,%.2f,%.2f", vector.x, vector.y, vector.z);

	return sVector;
}

inline Quat QuatFromString(const string& sQuat)
{
	const size_t xPos = sQuat.find(",");
	const size_t yPos = sQuat.find(",", xPos + 1);
	const size_t zPos = sQuat.find(",", yPos + 1);
...
inline string QuatToString(const Quat quaternion, bool isShort = true)
{
	string sQuaternion;
	if (!isShort)
		sQuaternion.Format("%f,%f,%f,%f", quaternion.w, quaternion.v.x, quaternion.v.y, quaternion.v.z);
	else
		sQuaternion.Format("%.2f,%.2f,%.2f,%.2f", quaternion.w, quaternion.v.x, quaternion.v.y, quaternion.v.z);
	return sQuaternion;
}

inline ColorF StringToColor(const char *sColor, const bool adjustGamma)
{
	ColorF color(1.f);
	...
	for (int i = 0; i < 4; ++i)
	{
		...
		const auto fToken = float(atof(sToken));
...
inline uint64 StringToMs(const string& time)
{
	const size_t secondsPos = time.find(":");
	const size_t msPos = time.find(":", secondsPos + 1);
	...
	const uint64 ms = atoi(sMS.c_str()) + (atoi(sSeconds.c_str()) * 1000) + (atoi(sMinutes.c_str()) * 60000);
	return ms;
}